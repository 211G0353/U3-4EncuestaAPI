    ...
    public class ListaEncuestaController : ControllerBase
    {
        ...
        public IActionResult GuardarRespuesta([FromBody] RespuestaDTO dto)
        {
            try
            {
                ...
                // Obtener o CREAR automáticamente la aplicación si no existe
                var aplicacion = AplicacionRepository.GetAll()
                    .FirstOrDefault(a => a.IdEncuesta == dto.encuestaId)
                    ?? new Aplicacionencuesta
                    {
                        IdEncuesta = dto.encuestaId,
                        IdUsuario = 1, // O usa el ID del usuario actual
                        FechaInicio = DateTime.UtcNow,
                        FechaFin = DateTime.UtcNow.AddYears(1) // 1 año de vigencia
                    };

                if (aplicacion.Id == 0) // Si es nueva
                    AplicacionRepository.Insert(aplicacion);

                // Guardar respuesta normalmente
        ...
        //        {
        //            var nuePregunta = new Pregunta
        //            {
        //                Texto = p,
        //                IdEncuesta = nuevaEncuesta.Id,
        //                Orden = orden++
        //            };
        //            PreguntaRepository.Insert(nuePregunta);
        //        }
        //    }
        //    return Ok(new { Id = nuevaEncuesta.Id });
        //}
        ...
        {
            // no maneja roles asi q lo comente u.u
            //var claimId = User.FindFirst("Id")?.Value;
            //var encuesta = EncuestaRepository.GetAll().FirstOrDefault(e => e.Id && e.IdUsuario == claimId);
